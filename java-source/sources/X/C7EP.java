package X;

/* JADX INFO: renamed from: X.7EP, reason: invalid class name */
/* JADX INFO: loaded from: classes5.dex */
public final class C7EP extends C7TT {
    public final boolean A00;
    public final boolean A01;

    public boolean equals(Object obj) {
        if (this != obj) {
            if (obj instanceof C7EP) {
                C7EP c7ep = (C7EP) obj;
                if (this.A00 != c7ep.A00 || this.A01 != c7ep.A01) {
                }
            }
            return false;
        }
        return true;
    }

    public int hashCode() {
        return C3D8.A00(C3D8.A01(this.A00), this.A01);
    }

    public String toString() {
        boolean z = this.A00;
        boolean z2 = this.A01;
        StringBuilder sbA08 = AnonymousClass000.A08();
        sbA08.append("OnHideShapePicker(hasUndo=");
        sbA08.append(z);
        return AbstractC32971bt.A0U(", notifyDoodleListeners=", sbA08, z2);
    }

    public C7EP(boolean z, boolean z2) {
        this.A00 = z;
        this.A01 = z2;
    }
}
