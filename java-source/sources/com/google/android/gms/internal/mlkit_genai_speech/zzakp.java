package com.google.android.gms.internal.mlkit_genai_speech;

import X.J27;
import X.J2C;

/* JADX INFO: loaded from: classes10.dex */
public final class zzakp extends zzagg implements zzaho {
    public static final zzakp zzb;
    public static volatile zzahv zze;
    public int zzf;
    public zzafd zzg = zzafd.zza;
    public zzaks zzh;
    public long zzi;
    public int zzj;
    public int zzk;

    static {
        zzakp zzakpVar = new zzakp();
        zzb = zzakpVar;
        zzagg.zzD(zzakp.class, zzakpVar);
    }

    @Override // com.google.android.gms.internal.mlkit_genai_speech.zzagg
    public final Object zzb(int i, Object obj, Object obj2) throws Throwable {
        zzahv zzahvVarA0K;
        int i2 = i - 1;
        if (i2 == 0) {
            return (byte) 1;
        }
        if (i2 == 2) {
            Object[] objArrA1Z = J27.A1Z();
            J2C.A1U(objArrA1Z);
            objArrA1Z[4] = "zzj";
            J27.A1K(objArrA1Z, 5, 6, zzako.zza);
            return J27.A0N(zzb, "\u0001\u0005\u0000\u0001\u0001\u0005\u0005\u0000\u0000\u0000\u0001ည\u0000\u0002ဉ\u0001\u0003ဂ\u0002\u0004᠌\u0003\u0005င\u0004", objArrA1Z);
        }
        if (i2 == 3) {
            return new zzakp();
        }
        Throwable th = null;
        if (i2 == 4) {
            return new zzakn(zzb);
        }
        if (i2 == 5) {
            return zzb;
        }
        if (i2 == 6) {
            zzahv zzahvVar = zze;
            if (zzahvVar != null) {
                return zzahvVar;
            }
            synchronized (zzakp.class) {
                try {
                    zzahvVarA0K = zze;
                    if (zzahvVarA0K == null) {
                        zzaeq zzaeqVar = zzaeq.$redex_init_class;
                        zzahvVarA0K = J27.A0K(zzb);
                        zze = zzahvVarA0K;
                    }
                } catch (Throwable th2) {
                    th = th2;
                }
            }
            return zzahvVarA0K;
        }
        throw th;
    }
}
